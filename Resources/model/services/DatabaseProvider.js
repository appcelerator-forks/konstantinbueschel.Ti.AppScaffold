...
function DatabaseProvider(databaseName) {
	...
	this.newsTable =				'news';
	this.newsCategoriesTable =		'news_categories';
	this.newsCategoriesMMTable =	'news_categories_mm';
	
	this.eventTable =				'events';
	this.eventCategoriesTable =		'event_categories';
	this.eventCategoriesMMTable =	'event_categories_mm';
	this.eventLocationTable =		'event_locations';
	
	this.imageGalleriesTable =		'image_galleries';
	
	this.faqTable =					'faq';
	this.faqCategoriesTable =		'faq_categories';
	this.faqCategoriesMMTable =		'faq_categories_mm';
...
DatabaseProvider.prototype.init = function() {
    ...
    // news table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.newsTable + ' (id INTEGER PRIMARY KEY UNIQUE, title TEXT, bodytext TEXT, bodytext_plain TEXT, shorttext TEXT, shorttext_plain TEXT, datetime INTEGER, timestamp INTEGER DEFAULT CURRENT_TIMESTAMP, images TEXT, categories TEXT, hidden INTEGER DEFAULT 0, deleted INTEGER DEFAULT 0, viewed INTEGER DEFAULT 0);');
	
	// news categories table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.newsCategoriesTable + ' (id INTEGER PRIMARY KEY UNIQUE, name TEXT);');
	
	// news categories mm table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.newsCategoriesMMTable + ' (news_id INTEGER, category_id INTEGER)');
	
	
	// events table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.eventTable + ' (id INTEGER PRIMARY KEY UNIQUE, title TEXT, start INTEGER, end INTEGER, allday INT DEFAULT 0, organizer_id INTEGER DEFAULT 0, category_ids TEXT, location_id INTEGER, description TEXT, description_plain TEXT, images TEXT, more_link TEXT, ticket_link TEXT, price_free INTEGER DEFAULT 0, ticket_prices TEXT, artist TEXT, afterwork INTEGER DEFAULT 0, starred INTEGER DEFAULT 0, tstamp INTEGER DEFAULT CURRENT_TIMESTAMP);');	
	
	// event categories table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.eventCategoriesTable + ' (id INTEGER PRIMARY KEY UNIQUE, title TEXT, tstamp INTEGER DEFAULT CURRENT_TIMESTAMP);');
	
	// event locations table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.eventLocationTable + ' (id INTEGER PRIMARY KEY UNIQUE, name TEXT, description TEXT, street TEXT, zip TEXT, city TEXT, latitude REAL, longitude REAL, tstamp INTEGER DEFAULT CURRENT_TIMESTAMP);');
	
	// events categories mm table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.eventCategoriesMMTable + ' (category_id INTEGER, event_id INTEGER);');
	
	
	// image galleries table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.imageGalleriesTable + ' (id INTEGER PRIMARY KEY UNIQUE, name TEXT, type TEXT, images TEXT, viewed INTEGER DEFAULT 0);');
	
	
	// faq table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.faqTable + ' (id INTEGER PRIMARY KEY UNIQUE, question TEXT, answer TEXT, category_ids TEXT);');
	
	// faq categories table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.faqCategoriesTable + ' (id INTEGER PRIMARY KEY UNIQUE, name TEXT);');
	
	// faq categories mm table
	this._connection.execute('CREATE TABLE IF NOT EXISTS ' + this.faqCategoriesMMTable + ' (category_id INTEGER, faq_id INTEGER);');
        
        
    // database scheme updates given? 
    var currentDatabaseVersion = require('/helpers/common/globals').global.currentDatabaseVersion;
    ...
    // DEBUG INFO
    Tools.info('DatabaseProvider -> init() - Database ' + this.activeDatabase + ' initialized');
...
DatabaseProvider.prototype.isDatabaseSchemeUpToDate = function(currentDatabaseVersion) {
    ...
    if (this.getCurrentDatabaseVersion() >= currentDatabaseVersion) {
        ...
        // DEBUG INFO
        Tools.info('DatabaseProvider -> database scheme is up to date');
    ...
    // DEBUG WARNING
    Tools.warn('DatabaseProvider -> database scheme is deprecated');
...
DatabaseProvider.prototype.updateDatabaseScheme = function(updateToVersion) {
    ...
    if (updateToVersion) {
        ...
        for (i; i < updateToVersion; i++) {
            ...
            // DEBUG INFO
            Tools.info('DatabaseProvider -> updateDatabaseScheme() - database will be updated to version ' + nextVersionToUpdateTo);
    ...
    if (updateToVersion && updateToVersion > 0) {
        ...
        switch (updateToVersion) {
            ...
            case 2:
                ...
                // DEBUG INFO
                Tools.info('DatabaseProvider -> alterDatabaseSchemeToVersion() - Database scheme updated to version ' + updateToVersion);
            ...
            default:
                ...
                // DEBUG WARNING
                Tools.warn('DatabaseProvider -> alterDatabaseSchemeToVersion() - Tried to update to not specified database scheme version! (Failed version update: ' + updateToVersion + ')');
...
DatabaseProvider.prototype.select = function(queryOptions){
    ...
    // DEBUG INFO
    Tools.info('DatabaseProvider -> execQuery() -> Query: ' + (select + from + where + groupBy + orderBy + limit));
...
DatabaseProvider.prototype.getDatabaseFile = function(databaseName) {
    ...
    var dbFullName = ((Tools.isAndroid) ? databaseName : ((_helpers.endsInSQL(databaseName)) ? databaseName : databaseName + '.sql'));
...
DatabaseProvider.prototype.renameDatabase = function(oldName, newName) {
    ...
    if (db.exists()) {
        ...
        newName = ((Tools.isAndroid) ? newName : ((_helpers.endsInSQL(newName)) ? newName : newName + '.sql'));
