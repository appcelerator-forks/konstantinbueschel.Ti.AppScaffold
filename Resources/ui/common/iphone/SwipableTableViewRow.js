...
function SwipableTableViewRow(args) {
	...
	this._swipeIndicatorWidth = Math.abs(Number(args.swipeIndicatorWidth));
	...
	delete args.swipeIndicatorWidth;
	...
	this._contentView.setLayout(this.viewProxy.layout || 'composite');
	...
	function _afterLayout(postlayoutEvent) {
		...
		var contentRect = self._contentView.getRect(),
		    minRowHeight = _stylesheet.swipableTableViewRow.minRowHeight;


		// set minimum height
		if (contentRect.height < minRowHeight) {

			contentRect.height = minRowHeight;
		}


		// set height for children
		self._actionButtonContainer.setHeight(contentRect.height);


		// set snap width that would be visible on row swipe
		self._snapWidth = self._actionButtonContainer.getRect().width;


		// set width with 1-2dp from right to visually show
		// that there are swipe for actions
		self._contentView.setWidth(contentRect.width - self._swipeIndicatorWidth);
		self._snapWidth -= self._swipeIndicatorWidth;
	...
	function _deleteRow(clickEvent) {
		...
		if (self._parent) {
			...
			var parent = (self._parent.viewProxy || self._parent),
				apiName = parent.apiName;


			if (apiName && apiName === 'Ti.UI.TableView') {

				parent.deleteRow(self.viewProxy);

				if (self._deleteCallback) {

					self._deleteCallback(self, self.viewProxy, self.recordData);
				}
			}
