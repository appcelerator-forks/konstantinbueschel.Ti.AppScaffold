...
// In the style of http://www.thomasfrank.se/xml_to_json.html but for
// Appcelerator with extras.
...
var traverseTree = function(node) {
	...
	if (node.hasChildNodes()) {
		for (var ch_index = 0; ch_index < node.childNodes.length; ch_index++) {
			...
			//Text Node
			if (ch.nodeType === 3 || ch.nodeType === ch.CDATA_SECTION_NODE) {
				...
				if (node.childNodes.length === 1 && !node.hasAttributes()) {
					...
					// parse images in cdata-section
					if (ch.nodeName == '#cdata-section') {
						
						var imgTagPattern = /<img[^>]+src="([http|https][^"]+)"[^>]*\/>/ig;
						
						part.images = imgTagPattern.exec(ch.textContent).slice(1);
					} 
					else {
